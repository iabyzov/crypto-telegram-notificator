...
func (s *TelegramWebhookHandler) handleSetAlert(message *tgbotapi.Message) {
...
func (s *TelegramWebhookHandler) formatAlertsList(userAlerts []alerts.PriceAlert) string {
...
func (s *TelegramWebhookHandler) handleDeleteAlert(message *tgbotapi.Message) {
	args := strings.Fields(message.CommandArguments())
	if len(args) != 1 {
